...
& from DanielNolte/FederatedDeepRegressionForests: https://github.com/DanielNolte/FederatedDeepRegressionForests
"""
import resnetNCI60
import torch
...
class FeatureLayer(nn.Sequential):
    def __init__(self,opt, model_type = 'resnet34', num_output = 256, 
                 input_size = 672, pretrained = False, 
                 gray_scale = False):
        """
        Args:
        ...
        model = resnetNCI60.ANNShallowDRF(opt)
        self.add_module('feat_extract', model)

    def get_out_feature_size(self):
        return self.num_output
class Tree(nn.Module):
    def __init__(self, depth, feature_length,opt, use_cuda = True):
        """
        Args:
        ...
        self.mean = np.ones((self.n_leaf,1))
        self.mu_cache = []
        ...
        self.covmat = np.array([np.eye(1) for i in range(self.n_leaf)])
        # also stores the inverse of the covariant matrix for efficiency
        self.covmat_inv = np.array([np.eye(1) for i in range(self.n_leaf)])
        ...
        if not use_cuda:
            ...
            self.mean = Parameter(torch.from_numpy(self.mean).type(torch.FloatTensor).cuda(device=opt.gpuid), requires_grad=False)
            self.covmat = Parameter(torch.from_numpy(self.covmat).type(torch.FloatTensor).cuda(device=opt.gpuid), requires_grad=False)
            self.covmat_inv = Parameter(torch.from_numpy(self.covmat_inv).type(torch.FloatTensor).cuda(device=opt.gpuid), requires_grad=False)
            self.factor = Parameter(torch.from_numpy(self.factor).type(torch.FloatTensor).cuda(device=opt.gpuid), requires_grad=False)            
    ...
    def forward(self, x, save_flag = False):
        ...
        if x.is_cuda and not self.feature_mask.is_cuda:
            self.feature_mask = self.feature_mask.cuda(device=self.opt.gpuid)
    ...
    def update_label_distribution(self,target_batch, check=False):
        ...
        with torch.no_grad():
            ...
            new_covmat = new_mean.data.new(self.n_leaf, 1, 1)
            new_covmat_inv = new_mean.data.new(self.n_leaf, 1, 1)
        ...
        self.cuda(device=self.opt.gpuid)
        return
    
class Forest(nn.Module):
    # a neural decision forest is an ensemble of neural decision trees
    def __init__(self, n_tree, tree_depth, feature_length,opt, use_cuda = True):
        ...
        for _ in range(n_tree):
            tree = Tree(tree_depth, feature_length,opt, use_cuda)
